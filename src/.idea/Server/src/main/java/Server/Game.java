...
public class Game {
   ...
   private static final List<RequestData> requestDataList = new ArrayList<>();
                ...
                public void run() {
                    ...
                    if (request.getRequestStatus() == RequestStatus.Accepted ||request.getRequestStatus() == RequestStatus.Rejected ){
                        ...
                        for (RequestData requestData : requestDataList) {
                            if(requestData.getRequestId() == request.getRequestId()){
                                requestDataList.remove(requestData);
                            }
                        }

                         */
                      //  requestDataList.remove(request.getRequestId());
                        try {
...
//                      send to sender...
    ...
    public void startGame() throws IOException {
      ...
      set.setUpQuestions(this);
        ...
        timer.schedule(task= new GameHelper(this), 0,5*1000); // 5 second
            ...
            if (q.getEnteredAnswers() == null){
                ...
                for (EnteredAnswer enteredAnswer : q.getEnteredAnswers()){
                    ...
                    if(enteredAnswer.getAnswerId()== q.getCorrectAnswer().getAnswerId()){
                        ...
                        if(q.getEnteredAnswers().indexOf(enteredAnswer) == 0 ){

                            q.getEnteredAnswers().get(indexnumber).setAnswerScore(10);

                        }else if(q.getEnteredAnswers().indexOf(enteredAnswer) == 1 ){
                            q.getEnteredAnswers().get(indexnumber).setAnswerScore(8);

                        }else if(q.getEnteredAnswers().indexOf(enteredAnswer) == 2 ){
                            q.getEnteredAnswers().get(indexnumber).setAnswerScore(6);

                        }else if(q.getEnteredAnswers().indexOf(enteredAnswer) == 3 ){
